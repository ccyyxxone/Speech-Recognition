...
void LangModel::count_sentence_ngrams(const vector<int>& wordList)
    ...
    for(n=1; n <= m_n; n++){
        for(i = m_n-n; i<=wordCnt-n; i++){
            ...
            if(m_predCounts.get_count(temp) == 1){
                temp.pop_back();
                m_histOnePlusCounts.incr_count(temp);
            }
            else{
                temp.pop_back();
            }
            m_histCounts.incr_count(temp);
...
double LangModel::get_prob_plus_delta(const vector<int>& ngram) const
    ...
    retProb = (m_predCounts.get_count(ngram) + m_delta) / (m_histCounts.get_count(temp) + m_delta * vocSize);
...
double LangModel::get_prob_witten_bell(const vector<int>& ngram) const
    ...
    if(m_n >= 1){
        ...
        temp0.clear();
        vector<int> temp1;
        temp1.push_back(ngram[m_n-1]);

        if(m_histCounts.get_count(temp0) != 0){
            double Pmle = (double)m_predCounts.get_count(temp1) / m_histCounts.get_count(temp0);
            retProb = (double)m_histCounts.get_count(temp0)*Pmle / (m_histCounts.get_count(temp0) + m_histOnePlusCounts.get_count(temp0)) + (double)m_histOnePlusCounts.get_count(temp0) / (m_histCounts.get_count(temp0) + (double)m_histOnePlusCounts.get_count(temp0)) / vocSize;
        }
    ...
    if(m_n >= 2){
        vector<int> ngram2;
        ngram2.push_back(ngram[m_n-2]);
        ngram2.push_back(ngram[m_n-1]);
        vector<int> temp2 = ngram2;
        temp2.pop_back();

        if(m_histCounts.get_count(temp2) != 0){
            double Pmle2;
            Pmle2 = (double)m_predCounts.get_count(ngram2) / m_histCounts.get_count(temp2);
            retProb = (double)m_histCounts.get_count(temp2)*Pmle2 / (m_histCounts.get_count(temp2) + m_histOnePlusCounts.get_count(temp2)) + (double)m_histOnePlusCounts.get_count(temp2) / (m_histCounts.get_count(temp2) + (double)m_histOnePlusCounts.get_count(temp2)) * retProb;
        }
    }

    if(m_n == 3){
        vector<int> temp3 = ngram;
        temp3.pop_back();

        if(m_histCounts.get_count(temp3) != 0){
            double Pmle3;
            Pmle3 = (double)m_predCounts.get_count(ngram) / m_histCounts.get_count(temp3);
            retProb = (double)m_histCounts.get_count(temp3)*Pmle3 / (m_histCounts.get_count(temp3) + m_histOnePlusCounts.get_count(temp3)) + (double)m_histOnePlusCounts.get_count(temp3) / (m_histCounts.get_count(temp3) + (double)m_histOnePlusCounts.get_count(temp3)) * retProb;
