...
void FrontEnd::do_window(const matrix<double>& inFeats,
    ...
    if(doHamming==true)
    {
        ...
        while(1){
            int sampleCount = 0;
            for(i=start;i<end;i++){
                outFeats(frameCount,sampleCount) = inFeats(i,0)*(0.54-0.46*cos(2 * M_PI * (i-start) / (sampPerWindow-1)));
                sampleCount ++;
            }
            frameCount++;
            start += sampShift;
            end += sampShift;
            if(frameCount==outFrameCnt)
                break;
        }
    }
    else
    {
        ...
        while(1){
            int sampleCount = 0;
            for(i=start;i<end;i++){
                outFeats(frameCount,sampleCount) = inFeats(i,0);
                sampleCount ++;
            }
            frameCount++;
            start += sampShift;
...
void FrontEnd::do_melbin(const matrix<double>& inFeats,
    ...
    maxF = 1127 * log(1 + 1/(2*samplePeriod)/700.0);
    eachF = maxF / (numBins+1);

    for(i=0;i<inFrameCnt;i++){
        for(j=0;j<outDimCnt;j++){
            outFeats(i,j) = 0;
            for(k=0;k<inDimCnt/2;k++){
                K = 1127 * log(1 + (k/(inDimCnt/2.0)/samplePeriod)/2.0/700.0);
                if((K >= j*eachF) && (K <= (j+1)*eachF))
                    H = (K-j*eachF) / eachF;
                else if((K >= (j+1)*eachF) && (K<=(j+2)*eachF))
                    H = ((j+2)*eachF - K) / eachF;
                else
                    H = 0;
                outFeats(i,j) = outFeats(i,j) + sqrt( pow(inFeats(i,2*k), 2) + pow(inFeats(i,2*k+1), 2) ) * H;
...
void FrontEnd::do_dct(const matrix<double>& inFeats,
    ...
    for(i=0;i<inFrameCnt;i++){
        for(j=0;j<outDimCnt;j++){
            temp = 0;
            for(k=0;k<inDimCnt;k++){
                temp = temp + (inFeats(i,k) * cos(M_PI * (j+1) * (k+0.5) / inDimCnt));
            }
            outFeats(i,j) = sqrt(2.0/inDimCnt) * temp;
        }
    }
