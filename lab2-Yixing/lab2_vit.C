...
double viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    chart(0,start_state).assign(0, -1);  // log 1 = 0, 100%
    ...
    for(i = 0; i < frmCnt; i++){
        for(j = 0; j < stateCnt; j++){
            ...
            for(arcIdx = 0; arcIdx < arcCnt; arcIdx++){
                ...
                gmmId = arc.get_gmm();
                LogProb = arc.get_log_prob() + chart(i, j).get_log_prob() + gmmProbs(i, gmmId); //arc's prob, state's prob, and gmm
                dstLogProb = chart(i+1, dstState).get_log_prob(); // destination state's prob (record the maximum possible prob)
                
                if (dstLogProb < LogProb)
                    chart(i+1, dstState).assign(LogProb, nowArcId);
            }
        }
    }
