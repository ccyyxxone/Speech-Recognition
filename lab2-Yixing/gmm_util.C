...
GmmStats::GmmStats(GmmSet& gmmSet, const map<string, string>& params) :
    m_params(params), m_gmmSet(gmmSet),
...
double GmmStats::add_gmm_count(unsigned gmmIdx, double posterior,
    ...
    for(i=0; i<dimCnt; i++){
        m_gaussStats1(gaussIdx, i) += posterior * feats[i];
        m_gaussStats2(gaussIdx, i) += posterior * feats[i] * feats[i];
    }
...
double GmmStats::update(const vector<GmmCount>& gmmCountList,
    ...
    for (unsigned cntIdx = 0; cntIdx < gmmCountList.size(); ++cntIdx)
        ...
        if ((curFrmIdx >= frmCnt) || (gmmIdx >= gmmCnt))
            throw runtime_error("Out-of-bounds frame index or GMM index "
                "in GMM count.");
...
void GmmStats::reestimate() const
    ...
    int i, j;
    double gMean, gVar;
    for(i=0; i<gaussCnt; i++){
        for(j=0; j<dimCnt; j++){
            gMean = m_gaussStats1(i, j) / m_gaussCounts[i];
            gVar = (m_gaussStats2(i, j) - 2 * m_gaussStats1(i, j) * gMean) / m_gaussCounts[i] + (gMean * gMean);
            m_gmmSet.set_gaussian_mean(i, j, gMean);
            m_gmmSet.set_gaussian_var(i, j, gVar);
...
void split_gmms(const GmmSet& srcGmmSet, GmmSet& dstGmmSet,
    ...
    for (int srcGaussIdx = 0; srcGaussIdx < srcGaussCnt; ++srcGaussIdx)
        ...
        for (int dimIdx = 0; dimIdx < dimCnt; ++dimIdx)
            {
            double curMean = srcGmmSet.get_gaussian_mean(
                srcGaussIdx, dimIdx);
            double curVar = srcGmmSet.get_gaussian_var(
                srcGaussIdx, dimIdx);
            double perturbMean = perturbFactor *
                sqrt((curVar > 0.0) ? curVar : 0.0);
            if (perturbMean < minPerturb)
                perturbMean = minPerturb;
            dstGmmSet.set_gaussian_mean(dstGaussIdx, dimIdx,
                curMean - perturbMean);
...
void expand_gmms_ci_to_cd(const GmmSet& srcGmmSet, GmmSet& dstGmmSet,
    ...
    for (int srcGmmIdx = 0; srcGmmIdx < srcGmmCnt; ++srcGmmIdx)
        ...
        for (int phoneGmmIdx = 0; phoneGmmIdx < phoneGmmCnt; ++phoneGmmIdx)
            ...
            for (int compIdx = 0; compIdx < compCnt; ++compIdx)
                {
                dstGmmSet.set_component_weight(dstGmmIdx, compIdx,
                    srcGmmSet.get_component_weight(srcGmmIdx, compIdx));
                dstGmmSet.copy_gaussian(dstGmmSet.get_gaussian_index(
                    dstGmmIdx, compIdx), srcGmmSet,
