...
double forward_backward(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    int start_state = graph.get_start_state();
    int i, j, arcIdx;
    int arcCnt;
    int RecordArcId, nowArcId;
    ...
    chart(0,start_state).set_forw_log_prob(0);

    for(i = 0; i < frmCnt; i++){
        for(j = 0; j < stateCnt; j++){
            
            arcCnt = graph.get_arc_count(j);
            RecordArcId = graph.get_first_arc_id(j);
            
            for(arcIdx = 0; arcIdx < arcCnt; arcIdx++){
                nowArcId = RecordArcId;

                Arc arc;

                //  Place arc with ID "arcId" in "arc"; set "arcId"
                //  to arc ID of the next outgoing arc.
                RecordArcId = graph.get_arc(nowArcId, arc);  //the arc of nowArcId -> arc, next arc's Id -> RecordArcId
                
                //  You can now access elements of the Arc "arc", e.g.,
                dstState = arc.get_dst_state();  // find the state which is linked by arc

                gmmId = arc.get_gmm();
                LogProb = arc.get_log_prob() + chart(i, j).get_forw_log_prob() + gmmProbs(i, gmmId);
                dstLogProb = chart(i+1, dstState).get_forw_log_prob();

                LogProbList.clear();
                LogProbList.push_back(LogProb);
                LogProbList.push_back(dstLogProb);
                LogProbSum = add_log_probs(LogProbList);
                
                chart(i+1, dstState).set_forw_log_prob(LogProbSum);
            }
        }
    }
    ...
    vector<int> final_state_list;
    vector<int> back_state_list;
    int final_state_count = graph.get_final_state_list(final_state_list);
    final_state_list.swap(back_state_list);
    int backStateSize = final_state_count;
    int k;

    for(k = 0; k < backStateSize; k++){
        chart(frmCnt, back_state_list[k]).set_back_log_prob(0);
    }

    for(i = frmCnt-1; i>=0; i--){
        for(j = 0; j < stateCnt; j++){
            arcCnt = graph.get_arc_count(j);
            RecordArcId = graph.get_first_arc_id(j);
            ...
            for(arcIdx=0; arcIdx < arcCnt; arcIdx++){
                nowArcId = RecordArcId;
                Arc arc;
                RecordArcId = graph.get_arc(nowArcId, arc);
                dstState = arc.get_dst_state();

                if(chart(i+1, dstState).get_back_log_prob() > -9999999){
                    gmmId = arc.get_gmm();
                    LogProb = arc.get_log_prob() + chart(i+1, dstState).get_back_log_prob() + gmmProbs(i, gmmId); //which gmm?
                    dstLogProb = chart(i, j).get_back_log_prob();
                    ...
                    chart(i, j).set_back_log_prob(LogProbSum);
                }
    ...
    double arcPosteriorLog, arcPosterior;
    ...
    for(i=0; i<frmCnt; i++){
        for(j=0; j<stateCnt; j++){
            ...
            for(arcIdx=0; arcIdx<arcCnt; arcIdx++){
                ...
                arcPosteriorLog = chart(i, j).get_forw_log_prob() + chart(i+1, dstState).get_back_log_prob() + arc.get_log_prob() + gmmProbs(i, arc.get_gmm()) - uttLogProb;
                arcPosterior = exp(arcPosteriorLog);

                gmmCountList.push_back(GmmCount(arc.get_gmm(), i, arcPosterior));
            }
        }
    }
...
double init_backward_pass(const Graph& graph, matrix<FbCell>& chart)
    ...
    for (int finalIdx = 0; finalIdx < finalCnt; ++finalIdx)
        ...
        if (curCell.get_forw_log_prob() == g_zeroLogProb)
            continue;
        double curLogProb = curCell.get_forw_log_prob() +
...
Lab2FbMain::Lab2FbMain(const map<string, string>& params) :
    ...
    if (!m_transCountsFile.empty())
        m_graph.read_word_sym_table(get_required_string_param(params,
            "trans_syms"));
    }

bool Lab2FbMain::init_iter()
    {
    if (m_iterIdx > m_iterCnt)
        return false;
    m_transCounts.clear();
    m_audioStrm.clear();
    m_audioStrm.open(get_required_string_param(m_params,
        "audio_file").c_str());
    m_graphStrm.clear();
    m_graphStrm.open(get_required_string_param(m_params,
        "graph_file").c_str());
    m_totFrmCnt = 0;
    m_totLogProb = 0.0;
    return true;
...
void Lab2FbMain::finish_iter()
    ...
    m_audioStrm.close();
    m_graphStrm.close();
    cout << format("Iteration %d: %.6f logprob/frame (%d frames)") %
        m_iterIdx % (m_totFrmCnt ? m_totLogProb / m_totFrmCnt : 0.0) %
        m_totFrmCnt << endl;
    ++m_iterIdx;
    }
...
void main_loop(const char** argv)
    ...
    while (mainObj.init_iter())
        ...
        while (mainObj.init_utt())
            {
            double logProb = forward_backward(mainObj.get_graph(),
