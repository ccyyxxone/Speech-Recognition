...
double do_viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    double nextHighest = g_zeroLogProb;
    ...
    for (int frmIdx = 0; frmIdx <= frmCnt; ++frmIdx){
        ...
        double kthHighestLogProb;
        kthHighestLogProb = g_zeroLogProb;
        ...
        if(beamStateCnt != 0){
            for (int cellIdx = 0; cellIdx < cellCnt; ++cellIdx){
                ...
                record.push_back(curLogProb);
            }
            sort(record.begin(), record.end());
            reverse(record.begin(), record.end());
            int Vsize = record.size();
            if(Vsize < beamStateCnt)
                kthHighestLogProb = record[Vsize-1];
            else
                kthHighestLogProb = record[beamStateCnt-1];
        }
        

        
        double curHighest = nextHighest;
        nextHighest = g_zeroLogProb;
        ...
        while (((curState = curFrame.get_next_state())) >= 0){
            ...
            //  Pruning.
            //Beam Pruning
            if(curCell.get_log_prob() < curHighest - beamLogProb)
                continue;

            //Rank Pruning
            if(beamStateCnt!=0){
                if(curCell.get_log_prob() < kthHighestLogProb)
                    continue;
            }
            ...
            for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                ...
                if (hasGmm == true){
                    ...
                    temp = arc.get_log_prob() + gmmProbs(frmIdx,arc.get_gmm()) * acousWgt + curCell.get_log_prob();
                    if(temp > nextHighest)
                        nextHighest = temp;
                    cellPointer = &(nextFrame.insert_cell(dstState));
                }
                else{
                    temp = arc.get_log_prob() + curCell.get_log_prob();
                    cellPointer = &(curFrame.insert_cell(dstState));
                }

                if(cellPointer -> get_log_prob() < temp){
                    if(arc.get_word() == 0)
                        dstWordTreeIdx = curCell.get_node_index();
                    else
                        dstWordTreeIdx = wordTree.insert_node(curCell.get_node_index(), arc.get_word());

                    cellPointer -> assign(temp, dstWordTreeIdx);
                }
